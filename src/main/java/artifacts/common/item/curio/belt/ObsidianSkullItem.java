...
public class ObsidianSkullItem extends TrinketArtifactItem {
	...
	private static void onLivingHurt(LivingEntity user, DamageSource source, float amount) {
		if (!user.level.isClientSide && amount >= 1 && source.isFire() && user instanceof Player player
				&& TrinketsHelper.isEquipped(ModItems.OBSIDIAN_SKULL, user) && !player.getCooldowns().isOnCooldown(ModItems.OBSIDIAN_SKULL)) {
			user.addEffect(new MobEffectInstance(MobEffects.FIRE_RESISTANCE, 600, 0, false, true));
			((Player) user).getCooldowns().addCooldown(ModItems.OBSIDIAN_SKULL, 1200);
		}
	}
