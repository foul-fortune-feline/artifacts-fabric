...
public class MimicModel extends EntityModel<MimicEntity> {
	...
	public void prepareMobModel(MimicEntity mimic, float limbAngle, float limbDistance, float tickDelta) {
		if (mimic.ticksInAir > 0) {
			upperTeeth.xRot = upperMouthOverlay.xRot = Math.max(-60, (mimic.ticksInAir - 1 + tickDelta) * -6) * 0.0174533F;
			lowerTeeth.xRot = lowerMouthOverlay.xRot = Math.min(30, (mimic.ticksInAir - 1 + tickDelta) * 3) * 0.0174533F;
		} else {
